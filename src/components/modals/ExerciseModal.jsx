...
function ExerciseModal({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (type === "edit") {
        if (!thumbnail) {
          await dispatch(updateExerciseLesson({ id: courseId, formData }));
        ...
        const file = await getCompressedImage(thumbnail);
        formData.append("files", file);
        await dispatch(addExerciseLesson({ id: courseId, formData }));
        setNavigateData((prev) => ({
          ...prev,
  ...
  const handleDeleteLesson = async () => {
    ...
    dispatch(deleteExerciseLesson({ id: courseId, lessonIndex }))
      .unwrap()
      .then((res) => {
        dispatch(fetchExercise())
          .unwrap()
