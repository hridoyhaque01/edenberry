...
import ExerciseDeletePopup from "../../components/modals/ExerciseDeletePopup";
...
import {
  ...
  fetchExercise,
  updateExercise,
} from "../../features/services/exerciseSlice";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (type === "edit") {
        ...
        dispatch(updateExercise({ id, formData }))
          .unwrap()
          .then((res) => {
            dispatch(fetchExercise())
              ...
              .then((res) => {
                ...
                setIsLoading(false);
                setSuccess(true);
                infoNotify("Exercise update successful");
              });
          })
          .catch((error) => {
            setIsLoading(false);
            setSuccess(false);
            errorNotify("Exercise update failed");
          });
      } else {
        const data = {
          title,
          ...
          .then((courseResult) => {
            dispatch(fetchExercise())
              ...
              .then((res) => {
                ...
                setIsLoading(false);
                setSuccess(true);
                infoNotify("Exercise add successful");
                setIsDisabled(true);
              });
          })
          .catch((err) => {
            setIsLoading(false);
            setSuccess(true);
            errorNotify("Exercise add failed");
          });
      }
    } catch (error) {
      setIsLoading(false);
    ...
    dispatch(deleteExercise(id))
      ...
      .then((res) => {
        dispatch(fetchExercise())
          .unwrap()
          .then((res) => {
            // infoNotify("Delete Exercise successfull");
            navigate("/services");
            setIsLoading(false);
          });
      })
      .catch((err) => {
        errorNotify("Delete Exercise failed");
        setIsLoading(false);
      });
  };
  ...
  return (
    <>
      <section className="pt-12 pb-10">
        <FormTitle
          path="/services"
          title={`${type === "edit" ? "Update" : "Add"} Exercise`}
        ></FormTitle>
              ...
              {navigateData?.lessons?.map((lesson, index) => (
                ...
                >
                  ...
                  <div>
                    <button
                      type="button"
                      data-hs-overlay="#exercise-modal"
